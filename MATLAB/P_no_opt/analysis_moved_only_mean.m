...
%参照するファイルの選択
cd C:\Users\sp_tamatsu\Documents\MATLAB\analysis\behavior\injury %data file path
...
% session→day
rule_day_data=NaN(rule_size*3,5);
frule_day_data = NaN(rule_size*3,5);
porking_day_data = NaN(rule_size*3,5);
laps_day_data=NaN(rule_size*3,5);
CR_day_data = NaN(rule_size*3,5);
pCR_day_data = NaN(rule_size*3,5);
p_rate_day_data = NaN(rule_size*3,5);
speed_day_data = NaN(rule_size*3,5);
for k=1:5
    for l=1:3
        rule_day_data((l-1)*rule_size+1:l*rule_size,k)=rule_all_data(1:rule_size,l+3*(k-1)); %SDに利用
        frule_day_data((l-1)*rule_size+1:l*rule_size,k)=frule_all_data(1:rule_size,l+3*(k-1));
        porking_day_data((l-1)*rule_size+1:l*rule_size,k)=porking_all_data(1:rule_size,l+3*(k-1)); 
        laps_day_data((l-1)*rule_size+1:l*rule_size,k)=laps_all_data(1:rule_size,l+3*(k-1));
        CR_day_data((l-1)*rule_size+1:l*rule_size,k)=CR_all_data(1:rule_size,l+3*(k-1)); 
        pCR_day_data((l-1)*rule_size+1:l*rule_size,k)=pCR_all_data(1:rule_size,l+3*(k-1));
        p_rate_day_data((l-1)*rule_size+1:l*rule_size,k)=p_rate_all_data(1:rule_size,l+3*(k-1));
        speed_day_data((l-1)*rule_size+1:l*rule_size,k)=speed_all_data(1:rule_size,l+3*(k-1));
    end
end
%計算処理
...
frule_day_ave =mean(frule_day_data,'omitnan');
frule_day_SD = std(frule_day_data,'omitnan');
...
function [rule_all_data, frule_all_data, porking_all_data, laps_all_data, CR_all_data, pCR_all_data, p_rate_all_data, speed_all_data] =fl_1(file,path)
    ...
    for k=2:n
        ...
        p_rate_all_data2 = filename{1,k}.p_rate;
        speed_all_data2 = filename{1,k}.s_speed;
